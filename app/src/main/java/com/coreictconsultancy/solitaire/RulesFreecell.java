...
class RulesFreecell extends Rules {
  ...
  public void Init(Bundle map) {
    ...
    for (int i = 0; i < 4; i++) {
      mCardAnchor[i] = com.coreictconsultancy.solitaire.CardAnchor.CreateAnchor(com.coreictconsultancy.solitaire.CardAnchor.FREECELL_HOLD, i, this);
    }

    // Top anchors for sinking cards
    for (int i = 0; i < 4; i++) {
      mCardAnchor[i+4] = com.coreictconsultancy.solitaire.CardAnchor.CreateAnchor(com.coreictconsultancy.solitaire.CardAnchor.SEQ_SINK, i+4, this);
    }
    ...
    for (int i = 0; i < 8; i++) {
      if ( ! mBySuit ) {
        ...
        mCardAnchor[i+8] = com.coreictconsultancy.solitaire.CardAnchor.CreateAnchor(com.coreictconsultancy.solitaire.CardAnchor.FREECELL_STACK, i+8,
                                                   this);
      } else {
        // Baker's Game - build by suit
        mCardAnchor[i+8] = com.coreictconsultancy.solitaire.CardAnchor.CreateAnchor(com.coreictconsultancy.solitaire.CardAnchor.GENERIC_ANCHOR, i+8, this);
        mCardAnchor[i+8].SetBuildSeq(com.coreictconsultancy.solitaire.GenericAnchor.SEQ_DSC);
        mCardAnchor[i+8].SetMoveSeq(com.coreictconsultancy.solitaire.GenericAnchor.SEQ_ASC);
        mCardAnchor[i+8].SetSuit(com.coreictconsultancy.solitaire.GenericAnchor.SUIT_SAME);
        mCardAnchor[i+8].SetWrap(false);
        mCardAnchor[i+8].SetPickup(com.coreictconsultancy.solitaire.GenericAnchor.PACK_LIMIT_BY_FREE);
        mCardAnchor[i+8].SetDropoff(com.coreictconsultancy.solitaire.GenericAnchor.PACK_LIMIT_BY_FREE);
        mCardAnchor[i+8].SetDisplay(com.coreictconsultancy.solitaire.GenericAnchor.DISPLAY_ALL);
      }
    }
    ...
    if (event == EVENT_STACK_ADD) {
      if (anchor.GetNumber() >= 4 && anchor.GetNumber() < 8) {
        if (mCardAnchor[4].GetCount() == 13 && mCardAnchor[5].GetCount() == 13 &&
          ...
          PostEarnings saveEarnings = new PostEarnings("1", "Freecell",mView.GetContext());
  ...
  @Override
  public void EventProcess(int event, com.coreictconsultancy.solitaire.CardAnchor anchor, Card card) {
    if (mIgnoreEvents) {
      anchor.AddCard(card);
      return;
    }
  ...
  private boolean TryToSinkCard(com.coreictconsultancy.solitaire.CardAnchor anchor, Card card) {
    for (int i = 0; i < 4; i++) {
      if (mCardAnchor[i+4].DropSingleCard(card)) {
