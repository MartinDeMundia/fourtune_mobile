...
class RulesTriPeaks extends Rules {
  ...
  public void Init(Bundle map) {
    ...
    for (int i = 0; i < 28; i++) {
      mCardAnchor[i] = com.coreictconsultancy.solitaire.CardAnchor.CreateAnchor(com.coreictconsultancy.solitaire.CardAnchor.GOLF_STACK, i, this);
    }

    // Top dealt from deck anchor to waste/sink anchor
    mCardAnchor[28] = com.coreictconsultancy.solitaire.CardAnchor.CreateAnchor(com.coreictconsultancy.solitaire.CardAnchor.DEAL_FROM, 28, this);
    if (!mWrapCards) {
      mCardAnchor[29] = com.coreictconsultancy.solitaire.CardAnchor.CreateAnchor(com.coreictconsultancy.solitaire.CardAnchor.GOLF_WASTE, 29, this);
    } else {
      mCardAnchor[29] = com.coreictconsultancy.solitaire.CardAnchor.CreateAnchor(com.coreictconsultancy.solitaire.CardAnchor.GOLF_WRAPCARDS_WASTE, 29, this);
    }
  ...
  public void Resize(int width, int height) {
    int rem = (width - (com.coreictconsultancy.solitaire.Card.WIDTH * 10)) / 11;
    int maxHeight = height - (20 + com.coreictconsultancy.solitaire.Card.HEIGHT);
    int vSpacing = Math.min( maxHeight / 5 - 20, com.coreictconsultancy.solitaire.Card.HEIGHT * 3 / 5);
    int x = -1;
    int y = -1;
    // Position three peaks cards
    x = rem*5/2 + com.coreictconsultancy.solitaire.Card.WIDTH*3/2;
    y = 20 + com.coreictconsultancy.solitaire.Card.HEIGHT;
    for (int i = 0; i < 3; i++) {
      mCardAnchor[i].SetPosition(x, y);
      x += rem*3 + com.coreictconsultancy.solitaire.Card.WIDTH*3;
    }
    // Position next six cards
    x = rem*2 + com.coreictconsultancy.solitaire.Card.WIDTH;
    y += vSpacing;
    for (int i = 3; i < 9; i++) {
      mCardAnchor[i].SetPosition(x, y);
      x += rem + com.coreictconsultancy.solitaire.Card.WIDTH;
      i++;  // Extra increment of loop counter
      mCardAnchor[i].SetPosition(x, y);
      x += rem*2 + com.coreictconsultancy.solitaire.Card.WIDTH*2;
    }
    // Position next nine cards
    x = rem*3/2 + com.coreictconsultancy.solitaire.Card.WIDTH/2;
    y += vSpacing;
    for (int i = 9; i < 18; i++) {
      mCardAnchor[i].SetPosition(x, y);
      x += rem + com.coreictconsultancy.solitaire.Card.WIDTH;
    ...
    for (int i = 28; i < 30; i++) {
      mCardAnchor[i].SetPosition(rem + (37 - i) * (rem + com.coreictconsultancy.solitaire.Card.WIDTH), 10);
    }
  ...
  public void EventProcess(int event, com.coreictconsultancy.solitaire.CardAnchor anchor) {
    ...
    if (event == EVENT_DEAL) {
      if (mCardAnchor[28].GetCount() > 0) {
        ...
        mMoveHistory.push(new com.coreictconsultancy.solitaire.Move(28, 29, 1, true, false));
      }
      ...
      if ((mCardAnchor[28].GetCount() + mCardAnchor[29].GetCount()) == 52) {
        ...
        PostEarnings saveEarnings = new PostEarnings("1", "TriPeaks",mView.GetContext());
    ...
    if (event == EVENT_SMART_MOVE) {
      ...
      for (int i = 0; i < 28; i++) {
        ...
        if (mCardAnchor[i].GetCount() > 0 && mCardAnchor[i].GetHiddenCount() <= 0) {
          com.coreictconsultancy.solitaire.Card card = mCardAnchor[i].PopCard();
  ...
  private boolean TryToSinkCard(com.coreictconsultancy.solitaire.CardAnchor anchor, com.coreictconsultancy.solitaire.Card card) {
    if (mCardAnchor[29].DropSingleCard(card)) {
      mMoveHistory.push(new com.coreictconsultancy.solitaire.Move(anchor.GetNumber(), 29, 1, false, anchor.UnhideTopCard()));
