 	...
 	public void loadGame() {
  		try {
			...
			if (map.bookMarks().size()>=Settings.level){
				...
				PostEarnings saveEarnings = new PostEarnings(""+Settings.level, "Super Mario",((MarioGame) game));
				String userId = saveEarnings.getLoggedUser();
				if (userId.equals("")) {
					Intent intent = new Intent(((MarioGame) game),com.coreictconsultancy.fourtune.ui.Signin.class);
					((MarioGame) game).startActivity(intent);
				}else{
