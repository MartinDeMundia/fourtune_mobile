...
package com.coreictconsultancy.mario.mario.preferences;
...
public class SetPreferencesActivity extends PreferenceActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getPreferenceManager().setSharedPreferencesName(PreferenceConstants.PREFERENCE_MAIN_NAME);
        
        // Load the preferences from an XML resource
        addPreferencesFromResource(R.xml.preferences_mario);
        ...
        if (configureKeyboardPref != null) {
        	...
        	config.setPrefs(getSharedPreferences(PreferenceConstants.PREFERENCE_MAIN_NAME, MODE_PRIVATE));
        	config.setContext(this);
        }
        
        if (getIntent().getBooleanExtra("moreInfo", false)) {
        	PreferenceScreen info = (PreferenceScreen)getPreferenceManager().findPreference("aboutScreen");
        	if (info != null) {
        		setPreferenceScreen(info);
        	}
        }
	...
	public void onDialogClosed(boolean positiveResult) {
		if (positiveResult) {
			//Erase HighScores
			SharedPreferences prefs = getSharedPreferences(PreferenceConstants.PREFERENCE_SCORE_NAME, MODE_PRIVATE);
			SharedPreferences.Editor editor = prefs.edit();
			editor.clear();
			editor.commit();
			//Erase Settings
			prefs = getSharedPreferences(PreferenceConstants.PREFERENCE_MAIN_NAME, MODE_PRIVATE);
			editor = prefs.edit();
			editor.clear();
			editor.commit();
			/*
	...
	public void onPause(){
		super.onPause();
		Settings.loadPreferences(getBaseContext());
	}
}
